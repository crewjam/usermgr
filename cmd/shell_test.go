...
var _ = Suite(&TestShell{})

func (s *TestShell) SetUpTest(c *C) {
...
func (s *TestShell) TearDownTest(c *C) {
	os.RemoveAll(s.tempDir)
}

func (s *TestShell) TestCanDoInteractiveLogin(c *C) {
	didExec := make(chan struct{})
	syscallExec = func(argv0 string, argv []string, envv []string) (err error) {
		c.Assert(argv0, Equals, "/usr/bin/sudo")
		c.Assert(argv, DeepEquals, []string{"sudo", "-u", "alice", "/bin/bash"})
		c.Assert(envv, IsNil)
		close(didExec)
		return nil
	}
	...
	c.Assert(err, IsNil)
	c.Assert(string(s.Output.Bytes()), Equals, "[shell logging active]\n")
	_, _ = <-didExec
}

func (s *TestShell) TestUserDoesntExist(c *C) {
	syscallExec = func(argv0 string, argv []string, envv []string) (err error) {
		panic("not reached")
		return nil
	}
	os.Setenv("USER", "badluckbrian")
	err := Main([]string{"usermgr", "--config=" + filepath.Join(s.tempDir, "usermgr.conf"),
		"shell"}, s.Output)
	c.Assert(err, ErrorMatches, "unknown user")
}

func (s *TestShell) TestCanDoMFA(c *C) {
	ioutil.WriteFile(filepath.Join(s.tempDir, "usermgr.conf"), []byte(""+
		fmt.Sprintf("CacheDir = %q\n", s.tempDir)+
	...
	defer func() { os.Args[0] = oldArg0 }()
	os.Args[0] = "-bash"
	stdin = bytes.NewBufferString("wrong\nwrong\nhellohellohelloo\n")

	err := Main([]string{"usermgr", "--config=" + filepath.Join(s.tempDir, "usermgr.conf"),
		"shell"}, s.Output)
	c.Assert(err, IsNil)
	c.Assert(string(s.Output.Bytes()), Equals, ""+
		"[shell logging active]\n"+
		"Multi-factor code: Incorrect token: invalid code\n"+
		"Multi-factor code: Incorrect token: invalid code\n"+
		"Multi-factor code: ")
	_, _ = <-didExec
}

func (s *TestShell) TestRejectsMFA(c *C) {
	ioutil.WriteFile(filepath.Join(s.tempDir, "usermgr.conf"), []byte(""+
		fmt.Sprintf("CacheDir = %q\n", s.tempDir)+
		"LoginMFARequried = true\n"+
		"HostKey = \"m_NiqMyWkkgOi1sT4uMCnp5kYuNanescRkRr3DP29FUAAgQGCAoMDhASFBYYGhweICIkJigqLC4wMjQ2ODo8Pg\"\n"), 0644)
	...
	err := Main([]string{"usermgr", "--config=" + filepath.Join(s.tempDir, "usermgr.conf"),
		"shell"}, s.Output)
	c.Assert(err, ErrorMatches, "Incorrect token")
	c.Assert(string(s.Output.Bytes()), Equals, ""+
		"[shell logging active]\n"+
		"Multi-factor code: Incorrect token: invalid code\n"+
		"Multi-factor code: Incorrect token: invalid code\n"+
...
func (s *TestShell) TestCanNonInteractiveCommand(c *C) {
	didExec := make(chan struct{})
	syscallExec = func(argv0 string, argv []string, envv []string) (err error) {
		c.Assert(argv0, Equals, "/usr/bin/sudo")
		c.Assert(argv, DeepEquals, []string{"sudo", "-u", "alice", "/bin/bash", "-c", "barg blerg"})
		c.Assert(envv, IsNil)
		close(didExec)
		return nil
	}
	...
	err := Main([]string{"usermgr", "--config=" + filepath.Join(s.tempDir, "usermgr.conf"),
		"shell", "-c", "barg blerg"}, s.Output)
	c.Assert(err, IsNil)
	c.Assert(string(s.Output.Bytes()), Equals, "")
	_, _ = <-didExec
}

func (s *TestShell) TestUpdateFallsNoCache(c *C) {
...
func (s *TestShell) TestBadConfig(c *C) {
