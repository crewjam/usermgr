...
var _ = Suite(&TestList{})

func (s *TestList) SetUpTest(c *C) {
...
func (s *TestList) TearDownTest(c *C) {
	os.RemoveAll(s.tempDir)
}

func (s *TestList) TestCanPrintKeys(c *C) {
	err := Main([]string{"usermgr", "--config=" + filepath.Join(s.tempDir, "usermgr.conf"),
		"list"}, s.Output)
	c.Assert(err, IsNil)
	c.Assert(string(s.Output.Bytes()), Equals, "alice\n")
}

func (s *TestList) TestUpdateFallsNoCache(c *C) {
...
func (s *TestList) TestBadConfig(c *C) {
